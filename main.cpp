...
public:
    ...
    {
        // Comparamos nombre y nivel del entrenador
        if (nombre != ec.nombre || nivel != ec.nivel)
            return false;

        // Comparamos el número de pokemones
        if (misPokemones.size() != ec.misPokemones.size())
            return false;

        // Comparamos cada Pokémon
        for (size_t i = 0; i < misPokemones.size(); ++i)
        {
            if (!(misPokemones[i] == ec.misPokemones[i])) // Uso del == sobrecargado en Pokémon
                return false;
        }
            ...
            do
            {
                try
                {
                    ...
                    else if (atq == 0 && tipoAtaque == 'p')
                    {
                        ...
                        atq++;
                        cout << miPok.getNombre() << "lanzo su ataque especial de " << miPok.getAtaqueEspecial() << endl;
                        cond = false;
                    }
                    else
                    {
